...
let
  ...
  command = name: properties: ''
    <action name="${name}">
      ${builtins.concatStringxSep "\n" properties}
    </action>
  '';

  join = contains: builtins.concatStringsSep "\n" contains;
in ''
    ...
    ${
      join [
        ...
        (props "keyboard" [
          ...
          (keybind "A-g" [ (exec "gyazo capture") ])
          (keybind "A-S-g" [ (exec "gyazo screenshot") ])

          (keybind "A-p" [ (exec "screenshot capture") ])
          (keybind "A-S-p" [ (exec "screenshot screenshot") ])
